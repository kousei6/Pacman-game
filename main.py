...
# WASM ではカレントディレクトリ変更が意味を持たないため未使用

async def fade_in_image(image: pg.Surface, screen: pg.Surface, duration: float = 2.0) -> None:
    """
...
def draw_difficulty_menu(screen: pg.Surface, current_index: int) -> None:
    """難易度選択メニューを1フレーム描画する。"""
    ...
    for i, item in enumerate(menu_items):
        color = colors[i] if i == current_index else (255, 255, 255)
        rendered.append(font_menu.render(item, True, color))

    total_width = sum(r.get_width() for r in rendered) + offset * 2
    x = WIDTH // 2 - total_width // 2

    for r in rendered:
        screen.blit(r, (x, y_base))
        x += r.get_width() + offset
...
class Enemy(pg.sprite.Sprite):
    ...
    def update(self) -> None:
        ...
        current_time = time.time()
        if self.is_pausing:
            if current_time - self.pause_start >= self.pause_duration:
                self.is_pausing = False
            else:
                return
        if self.is_reviving:
...
async def main():
    ...
    start = True
    start_state = "WAIT_ENTER"  # WAIT_ENTER -> SELECT_DIFF -> GAME
    difficulty = None
    difficulty_index = 0
    game_clear = False
    tmr = 0
    ...
    while True:
        ...
        if start:
            ...
            if start_state == "WAIT_ENTER":
                draw_start_screen(screen)

                for event in events:
                    if event.type == pg.KEYDOWN and event.key == pg.K_RETURN:
                        start_state = "SELECT_DIFF"

            elif start_state == "SELECT_DIFF":
                draw_difficulty_menu(screen, difficulty_index)
